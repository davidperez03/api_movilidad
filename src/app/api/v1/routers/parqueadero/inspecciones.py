...
from app.application.use_cases.parqueadero.aprobar_inspeccion import AprobarInspeccionUseCase, ComandoAprobarInspeccion
...
@router.get("", response_model=PaginaResponse[InspeccionResponse])
async def listar_inspecciones(
    ...
    session: AsyncSession = Depends(get_session),
    _: Usuario = Depends(requiere_permiso("parqueadero.reportes:leer")),
    org_id: UUID | None = Depends(get_organization_id),
):
    ...
    return PaginaResponse(items=[_map(ins) for ins in pagina.items], siguiente_cursor=pagina.siguiente_cursor, total=pagina.tamanio)
...
async def aprobar_inspeccion(
    ...
    ins = await AprobarInspeccionUseCase(InspeccionRepositorioSQL(session)).ejecutar(
        ComandoAprobarInspeccion(
            inspeccion_public_id=public_id,
            es_apto=body.es_apto,
            firma_operador=body.firma_operador,
            firma_inspector=body.firma_inspector,
            observaciones=body.observaciones,
            actor_id=usuario.id,
        )
    )
